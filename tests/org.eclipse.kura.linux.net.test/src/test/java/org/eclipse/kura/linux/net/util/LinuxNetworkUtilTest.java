...
public class LinuxNetworkUtilTest {
    ...
    @Test
    public void shouldDetectHasAddressWithIPv4Only() throws KuraException {
        givenLinuxNetworkUtilWithMockedIpAddrShow();
        givenInterfaceWithIPv4Only("eth0");

        whenCheckingHasAddress("eth0");

        thenInterfaceHasAddress(true);
    }

    @Test
    public void shouldDetectHasAddressWithIPv6Only() throws KuraException {
        givenLinuxNetworkUtilWithMockedIpAddrShow();
        givenInterfaceWithIPv6Only("eth0");
    ...
    @Test
    public void shouldDetectHasAddressWithBothIPv4AndIPv6() throws KuraException {
        givenLinuxNetworkUtilWithMockedIpAddrShow();
        givenInterfaceWithBothIPv4AndIPv6("eth0");
    ...
    @Test
    public void shouldDetectNoAddressWhenNeitherIPv4NorIPv6() throws KuraException {
        givenLinuxNetworkUtilWithMockedIpAddrShow();
        givenInterfaceWithNoAddresses("eth0");

        whenCheckingHasAddress("eth0");

        thenInterfaceHasAddress(false);
    }

    @Test
    public void shouldReturnIPv4AddressWhenBothAvailable() throws KuraException {
        givenLinuxNetworkUtilWithMockedIpAddrShow();
        givenInterfaceWithBothIPv4AndIPv6("eth0");

        whenGettingCurrentIPAddress("eth0");

        thenCurrentIPAddressIs("192.168.1.100");
    }

    @Test
    public void shouldReturnIPv6AddressWhenOnlyIPv6Available() throws KuraException {
        givenLinuxNetworkUtilWithMockedIpAddrShow();
        givenInterfaceWithIPv6Only("eth0");

        whenGettingCurrentIPAddress("eth0");

        thenCurrentIPAddressIs("2001:db8::1");
    }

    @Test
    public void shouldReturnNullWhenNoAddressesAvailable() throws KuraException {
        givenLinuxNetworkUtilWithMockedIpAddrShow();
        givenInterfaceWithNoAddresses("eth0");

        whenGettingCurrentIPAddress("eth0");

        thenCurrentIPAddressIsNull();
    }

    private boolean interfaceHasAddress;
    private String currentIPAddress;
    private LinuxNetworkUtil mockLinuxNetworkUtil;

    private void givenLinuxNetworkUtilWithMockedIpAddrShow() {
        CommandStatus status = new CommandStatus(new Command(new String[] {}), new LinuxExitStatus(0));
        this.commandExecutorServiceStub = new CommandExecutorServiceStub(status);
        this.mockLinuxNetworkUtil = mock(LinuxNetworkUtil.class);
    }

    private void givenInterfaceWithIPv4Only(String interfaceName) throws KuraException {
        LinuxIfconfig config = new LinuxIfconfig(interfaceName);
        config.setInetAddress("192.168.1.100");
        config.setInetMask("255.255.255.0");
        // No IPv6 address set
        when(this.mockLinuxNetworkUtil.getInterfaceConfiguration(interfaceName)).thenReturn(config);
    }

    private void givenInterfaceWithIPv6Only(String interfaceName) throws KuraException {
        LinuxIfconfig config = new LinuxIfconfig(interfaceName);
        // No IPv4 address set
        config.setInet6Address("2001:db8::1");
        config.setInet6Prefix(64);
        when(this.mockLinuxNetworkUtil.getInterfaceConfiguration(interfaceName)).thenReturn(config);
    }

    private void givenInterfaceWithBothIPv4AndIPv6(String interfaceName) throws KuraException {
        LinuxIfconfig config = new LinuxIfconfig(interfaceName);
        config.setInetAddress("192.168.1.100");
        config.setInetMask("255.255.255.0");
        config.setInet6Address("2001:db8::1");
    ...
    private void givenInterfaceWithNoAddresses(String interfaceName) throws KuraException {
        LinuxIfconfig config = new LinuxIfconfig(interfaceName);
        // No addresses set
        when(this.mockLinuxNetworkUtil.getInterfaceConfiguration(interfaceName)).thenReturn(config);
    }

    private void whenCheckingHasAddress(String interfaceName) throws KuraException {
        this.interfaceHasAddress = this.mockLinuxNetworkUtil.hasAddress(interfaceName);
    }

    private void whenGettingCurrentIPAddress(String interfaceName) throws KuraException {
        this.currentIPAddress = this.mockLinuxNetworkUtil.getCurrentIpAddress(interfaceName);
    }

    private void thenInterfaceHasAddress(boolean expected) {
        assertEquals(expected, this.interfaceHasAddress);
    }

    private void thenCurrentIPAddressIs(String expected) {
        assertEquals(expected, this.currentIPAddress);
    }

    private void thenCurrentIPAddressIsNull() {
        assertNull(this.currentIPAddress);
    }
