...
public class NMDeviceCreationHandler implements DBusSigHandler<NetworkManager.DeviceAdded> {
    ...
    public NMDeviceCreationHandler(
            NMDbusConnector connector, String deviceToCreate, BlockingQueue<Device> createdDeviceQueue) {
    ...
    public void handle(NetworkManager.DeviceAdded s) {
        try {
            ...
            if (deviceToCreate.equals(deviceId)) {
                ...
                Device createdDevice = nm.getDbusConnection()
                        .getRemoteObject("org.freedesktop.NetworkManager", createdDevicePath, Device.class);
