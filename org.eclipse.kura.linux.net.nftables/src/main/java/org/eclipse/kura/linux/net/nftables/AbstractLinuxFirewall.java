...
/**
 * Mid-level firewall API, mirroring the public method contract of the
 * iptables-era {@code AbstractLinuxFirewall} exactly, but backed by
 * {@link NftablesConfig} (live nftables application) and
 * {@link NftPersistenceStore} (restart-restore) instead of iptables-restore
 * text files.
 *
 * <p>
 * IPv4 and IPv6 are handled by two independent concrete subclasses
 * ({@link LinuxFirewall}, {@link LinuxFirewallIPv6}), each with its own
 * in-memory rule sets, its own nft table ({@code ip kura} / {@code ip6
 * kura}), and its own persistence file - mirroring the independence the
 * iptables-era {@code LinuxFirewall}/{@code LinuxFirewallIPv6} pair had, so
 * neither family's configuration can ever affect the other's.
 */
public abstract class AbstractLinuxFirewall {
    ...
    protected NftablesConfig nftablesConfig;
    protected NftPersistenceStore persistenceStore;

    protected AbstractLinuxFirewall(NftExecutor nftExecutor) {
        this.nftablesConfig = new NftablesConfig(nftExecutor, getFamily());
        this.persistenceStore = new NftPersistenceStore(getPersistenceFileName(), getPersistenceTmpFileName());
    }
    ...
    protected abstract String getPersistenceTmpFileName();

    protected abstract String getIpForwardFileName();

    protected abstract IPAddress getDefaultAddress() throws UnknownHostException;

    protected void setNftExecutor(NftExecutor nftExecutor) {
        this.nftablesConfig = new NftablesConfig(nftExecutor, getFamily());
    }

    public void initialize() throws KuraException {
        logger.debug("initialize() :: initializing firewall ...");
        Optional<NftRulesetModel> model = this.persistenceStore.load();
        if (model.isPresent()) {
            this.localRules = model.get().getLocalRules();
            this.portForwardRules = model.get().getPortForwardRules();
            this.autoNatRules = model.get().getAutoNatRules();
            this.natRules = model.get().getNatRules();
        }
        this.allowIcmp = true;
    ...
    public void addLocalRule(int port, String protocol, String permittedNetwork, String permittedNetworkPrefix,
        ...
        try {
            ...
            List<LocalRule> locRules = new ArrayList<>();
    ...
    public void addPortForwardRule(String inboundIface, String outboundIface, String address, String protocol,
        ...
        List<PortForwardRule> portFwdRules = new ArrayList<>();
    ...
    public void addNatRule(String sourceInterface, String destinationInterface, boolean masquerade, RuleType type)
        ...
        List<NATRule> natRuleList = new ArrayList<>();
    ...
    public void disable() throws KuraException {
        this.nftablesConfig.clearAllChains();
    }
    ...
    private void applyRules() throws KuraException {
        this.nftablesConfig.applyRules();
        if (this.portForwardRules != null && !this.portForwardRules.isEmpty()
            ...
            enableIpForwarding(getIpForwardFileName());
        }
    }

    private void enableIpForwarding(String fileName) {
        try (FileWriter fw = new FileWriter(fileName)) {
            fw.write('1');
        } catch (IOException e) {
            logger.warn("Failed to enable ip forwarding via {}", fileName, e);
        }
    }

    private void persistModel() throws KuraException {
        NftRulesetModel model = new NftRulesetModel();
        model.setLocalRules(this.localRules);
        model.setPortForwardRules(this.portForwardRules);
        model.setAutoNatRules(this.autoNatRules);
        model.setNatRules(this.natRules);
        model.setAllowIcmp(this.allowIcmp);
        model.setAdditionalFilterRules(this.additionalFilterRules);
        model.setAdditionalNatRules(this.additionalNatRules);
        model.setAdditionalMangleRules(this.additionalMangleRules);
        this.persistenceStore.save(model);
    }

    private void updateNftablesConfig() {
        this.nftablesConfig.setLocalRules(this.localRules);
        this.nftablesConfig.setPortForwardRules(this.portForwardRules);
        this.nftablesConfig.setNatRules(this.natRules);
        this.nftablesConfig.setAutoNatRules(this.autoNatRules);
        this.nftablesConfig.setAdditionalFilterRules(this.additionalFilterRules);
        this.nftablesConfig.setAdditionalNatRules(this.additionalNatRules);
        this.nftablesConfig.setAdditionalMangleRules(this.additionalMangleRules);

        // Mirrors the pre-existing iptables-era behavior: only custom-icmp-rule
        // presence gates the canned ICMP allow-list, the allowIcmp()/disableIcmp()
        // toggle is otherwise not consulted here.
        boolean customIcmpRulesExist = hasIcmpRulesInAdditionalRules();
        this.nftablesConfig.setAllowIcmp(!customIcmpRulesExist);
    }

    private boolean hasIcmpRulesInAdditionalRules() {
        return hasIcmpRules(this.additionalFilterRules) || hasIcmpRules(this.additionalNatRules)
    ...
    private boolean hasIcmpRules(Set<String> rules) {
        ...
        for (String rule : rules) {
            if (rule.toLowerCase().contains("icmp")) {
                return true;
            }
        }
        return false;
